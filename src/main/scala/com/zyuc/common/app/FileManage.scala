...
object FileManage {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("test_201802031140").setMaster("local[2]")
    ...
    val fromDirPath = new Path(inputPath + childPath + "/*")
    fileSystem.globStatus(fromDirPath).foreach(x => {
