...
/*
  * Created by hadoop on 17-12-14.
  * @author liuzk
  */
object CoalesceLogETL {
  def main(args: Array[String]): Unit = {
    val sparkConf = new SparkConf().setMaster("local[3]").setAppName("name_201712101130")
    val sc = new SparkContext(sparkConf)
    val sqlContext = new HiveContext(sc)


    val appName = sc.getConf.get("spark.app.name")//,"name_201712101130")
    ...
    FileUtils.move2Temp(fileSystem, inputPath, dataTime)
