...
object AuthLogETL {
  ...
  def doETL(sc:SparkContext, sqlContext: SQLContext, fileSystem:FileSystem, loadTime:String, inputDir:String, outputDir:String) : String = {
    println("inputDir:" + inputDir)
    println("outputDir:" + outputDir)
    val coalesceSize = 1
    val accessTable3g = "iot_etl_data_auth_3g"
    val accessTable4g = "iot_etl_data_auth_4g"
    val accessTableVpdn = "iot_etl_data_auth_vpdn"

    val wild3g = "3g*"
    val wild4g = "4g*"
    val wildVpdn = "vpdn*"

    val inputPath = inputDir + loadTime + "/"

    val dataPath = inputDir + loadTime + "_doing/"
    val lastDataPath = inputPath + loadTime + "_done/"
    ...
    def convert(dataPathWild:String,netType:String,accessTable:String) : Unit = {
      ...
      val outputPath = outputDir + netType + "/"
